...
model1<-lm(formula=dbh_mm~Age+si+Age*si,data=qaspen)
model2<-lm(formula=dbh_mm~Age+si+Age*si+0,data=qaspen)
summary(model1)
summary(model2)
...
best_model<-lm(formula=dbh_mm~Age+si+Age*si+0,data=qaspen)
summary(best_model)
