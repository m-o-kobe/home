...
fire_sp<-subset(fire4,fire4$sp.!="Sos")
fire_sp<-subset(fire_sp,!is.na(fire_sp$fire))

summary(fire4$sp.)
  
  
model1<-glm(formula=da~fire*dbh0*sp.,data=fire_sp,family="binomial")
model2<-glm(formula=da~fire*dbh0*sp.+0,data=fire_sp,family="binomial")
...
model3<-glm(formula=da~fire*dbh0+0,data=fire_sp,family="binomial")
summary(model3)

outcsv<-paste("mortality/mortality_fire1103.csv",sep="_")
write.csv(model_hikaku,outcsv)
tablecsv(model3,fire_sp,outcsv)
