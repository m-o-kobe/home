...
fire21<-subset(fire3,fire3$da==1)
...
pppfire21<-ppp(fire21$xx,fire21$yy,window=owin(c(0,90),c(0,100)),marks=fire21$D.A..2000.)
denfire21<-density(pppfire21,sigma=5.8,eps=c(1,1))



tc <- colourmap(matlab.like2(100), breaks=seq(0.0,100.0,length=101))
tc1 <- colourmap(matlab.like(30), breaks=seq(0.0,0.31,length=31))
tc2 <- colourmap(matlab.like(30), breaks=seq(0.0,0.1,length=31))
ch<-colourmap(col=colormap = "chlorophyll", breaks=seq(0.0,0.31,length=31))
cl<-colormap(colormap = "chlorophyll")
ch<-colourmap(cl(30), breaks=seq(0.0,0.31,length=31))
cl<-colorRampPalette(c("white","yellow", "greenyellow","green","darkgreen","navyblue","black"))
cr <- colourmap(cl(50),range=c(0.0,0.31))
cl(20)

plot(denfire1,col=cr,axes=TRUE,main=title,asp=1)
plot(denfire2,col=cr,axes=TRUE,main=title,asp=1)
plot(denfire21,col=cr,axes=TRUE,main="seizonritu",asp=1)

plot(100*denfire2/denfire1,col=tc,axes=TRUE,main=title,asp=1)
...
fire4<-subset(fire1,fire1$xx>0.0 && fire1$xx<90.0 && fire1$yy>0.0 &&fire1$yy<100.0)
fire4$fire<-0
len<-nrow(fire4)
...
fire5<-subset(fire4,fire4$sp.!="Sos")
g<-ggplot(data=fire5,aes(x=dbh0,y=fire,color=sp.,shape=D.A..2000.))+
  scale_shape_manual(values=c(1,4))+
  ggtitle("mortality")+xlab("DBH(cm)")+ylab("fire_intensity")+
  theme_bw()+
  geom_point(size=4)
print(g)
