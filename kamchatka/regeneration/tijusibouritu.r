...
#1年毎
bunkatu1<-cut(ketugou$age,breaks=seq(0,240,by=1))
bunkatu2<-table(bunkatu1)
bunkatu3<-as.data.frame(bunkatu2)
bunkatu3$age<-c(0:239)
bunkatu3<-subset(bunkatu3,bunkatu3$age>1)
bunkatu3<-subset(bunkatu3,bunkatu3$age<40)
...
#可視化
x_forplot <- data.frame(x=seq(0, 120, by=1))
...
int_bp2<-subset(int_bp,int_bp$dbh01<2.0)
summary(int_bp2)
